  ...
  async analyzeWallet(
    ...
    const totals = await transactions.reduce(
      async (acc, tx) => {
        ...
        if (isIncoming) {
          (await acc).totalIncoming += value * ethPrice;
        } else {
          (await acc).totalOutgoing += value * ethPrice;
        }

        (await acc).totalGasFees += gasCost * ethPrice;
    ...
    await Promise.all(
      transactions.map(async (tx) => {
        ...
        transaction.gasUsed = parseFloat(tx.gasUsed);
        transaction.gasPrice = parseFloat(
        ...
        // transaction.analysis = savedAnalysis[0];
        transaction.balance = tx.balance; // Ajouter le solde calculé précédemment
