...
export const Route = createFileRoute("/_authenticated/dashboard")({
  ...
  loader: async ({context}) => {
    ...
    const transactions = await fetch(`${API_URL}/wallet-analysis/transactions-summary?walletAddress=${walletAddress}&currency=${currency}`)
    const transactionsData = await transactions.json()

    const startDate = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString()
    const endDate = new Date().toISOString()

    const chart = await fetch(`${API_URL}/wallet-analysis/portfolio-history?walletAddress=${walletAddress}&currency=${currency}&startDate=${startDate}&endDate=${endDate}`)
